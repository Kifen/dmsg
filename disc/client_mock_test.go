	...
	cases := []struct {
	...
	}{
		{
			...
			databaseAndEntriesPrehook: func(t *testing.T, mockClient disc.APIClient, entries *[]*disc.Entry) {
				...
				err = mockClient.SetEntry(context.TODO(), &entry1, http.MethodPost)
				require.NoError(t, err)
				...
				err = entry2.Sign(sk1)
				require.NoError(t, err)
				err = mockClient.SetEntry(context.TODO(), &entry2, http.MethodPost)
				require.NoError(t, err)
				...
				err = entry3.Sign(sk2)
				require.NoError(t, err)
				err = mockClient.SetEntry(context.TODO(), &entry3, http.MethodPost)
				require.NoError(t, err)
	...
	cases := []struct {
	...
	}{
		{
			...
			storerPreHook: func(t *testing.T, apiClient disc.APIClient, e *disc.Entry) {
				err := apiClient.SetEntry(context.TODO(), e, http.MethodPost)
				require.NoError(t, err)
			},
		},
		{
...
func TestNewMockSetEntriesEndpoint(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			storerPreHook: func(t *testing.T, s disc.APIClient, e *disc.Entry) {
				...
				require.NoError(t, err)
				err = s.SetEntry(context.TODO(), &oldEntry, http.MethodPost)
				require.NoError(t, err)
				e.Sequence = 1
				e.Timestamp += 3
				err = e.Sign(sk)
		...
		},
		{
			...
			storerPreHook: func(t *testing.T, s disc.APIClient, e *disc.Entry) {
				e.Sequence = 2
				err := s.SetEntry(context.TODO(), e, http.MethodPost)
				require.NoError(t, err)
			},
		},
		{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := clientServer.SetEntry(context.TODO(), &entry, http.MethodPost)
...
func TestNewMockUpdateEntriesEndpoint(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			clientServer := disc.NewMock()
			err := clientServer.SetEntry(context.TODO(), &baseEntry, http.MethodPost)
			require.NoError(t, err)
			...
			err = clientServer.UpdateEntry(context.TODO(), tc.secretKey, &entry, nil)
...
func TestNewMockUpdateEntrySequence(t *testing.T) {
	...
	err := clientServer.UpdateEntry(context.TODO(), sk, entry, nil)
	require.NoError(t, err)
